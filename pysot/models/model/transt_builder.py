...
class TransTBuilder(BaseModelBuilder):
    ...
    def forward_original(self, data):
        ...
        with torch.no_grad():
            ...
            if self.enhance_neg > 0:
                neg_weight = sofTmax(score, T=0.20, b=0.2, mask=neg_mask, average='sample')
            else:
                neg_weight = label2weight(neg_mask, avg='sample')

            if self.enhance_pos > 0:
                pos_weight_cls = sofTmax(iou, T=0.25, b=0.7, mask=pos_mask, average='batch')
            else:
                pos_weight_cls = label2weight(pos_mask, avg='batch')

            if self.enhance_loc > 0:
                loc_weight = pos_mask + _pos_mask * iou
                pos_weight_loc = sofTmax(loc_weight, T=0.16, b=0., mask=bbox_mask, average='batch')
            else:
        ...
        self.enhance_iou = -1
        self.enhance_pos = -1
        if self.enhance_neg <= 0 and neg_loss < 0.06:
            self.enhance_neg += 1
            if self.enhance_neg > 0:
                print('enhance neg loss')
        if self.enhance_pos <= 0 and pos_loss < 0.08:
            self.enhance_pos += 1
            if self.enhance_pos > 0:
                print('enhance pos loss')
        if self.enhance_iou <= 0 and l1_loss < 12:
            self.enhance_iou += 1
            if self.enhance_iou > 0:
                print('enhance loc loss')
