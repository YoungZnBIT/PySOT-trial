...
def load_pretrain(model, pretrained_path, cuda=True):
...
def restore_from(model, optimizer, ckpt_path, scheduler=None):
    device = torch.cuda.current_device()
    ...
    if scheduler is None:
        ...
        check_keys(scheduler, ckpt['scheduler'])
        scheduler.load_state_dict(ckpt['scheduler'])
        return model, optimizer, epoch, scheduler


def resume(model, ckpt, optimizer, scheduler=None):
    ckpt_model_dict = remove_prefix(ckpt['state_dict'], 'module.')
    check_keys(model, ckpt_model_dict)
    model.load_state_dict(ckpt_model_dict, strict=False)
