...
def label_update_point(label, mask, score, iou, positive,
                       pos_iou_thresh=0.75, pos_num=16, temp_pos_num=8,
                       neg_iou_thresh=0.4, neg_score_thresh=0.25, neg_num=48, easy_neg_num=16, hard_neg_num=32):
    batch = label.shape[0]
    new_label = np.ones_like(label)
    positive_ = positive[:, None, None]
    ...
    neg_mask = label < 0.
    neg_mask = neg_mask.astype(np.float32)
    negative_ = (positive_ == 0.).astype(np.float32)
    neg_mask = neg_mask + negative_ * np.ones_like(neg_mask)
    neg_mask = neg_mask > 0.
    neg_mask = neg_mask.astype(np.float32)

    neg_mask_ = neg_mask == 0.
    neg_mask_ = neg_mask_.astype(np.float32)

    neg_label = np.zeros_like(label)
    # 正样本范围外且得分高于阈值的box
    neg_score = score * neg_mask
    neg = neg_score >= neg_score_thresh
    neg = neg.astype(np.float32)

    # 正样本范围外且IOU小于于阈值的box
    # 负样本对时不考虑IOU条件
    neg_iou = iou * neg_mask + neg_mask_ - negative_ * 5. * np.ones_like(neg_mask)
    easy_neg = neg_iou < neg_iou_thresh
    easy_neg = easy_neg.astype(np.float32)
    # 两者的交集为异常高得分，将作为难负样本
    hard_neg = neg * easy_neg
...
def guided_assign(label, mask, positive,
                  teacher_score, teacher_iou,
                  student_score, student_iou):
    pass
