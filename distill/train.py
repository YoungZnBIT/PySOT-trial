...
from distill.distiller import FitNetDistiller
...
parser.add_argument('--config', default='distill/mobilesiam-test1/mobilesiam_distill.yaml', type=str, help='config file')
...
def build_opt_lr(model):
    ...
    for m in model.teacher_model.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.eval()

    # 学生模型的backbone, 先全部冻结, 再按需解冻
    for param in model.student_model.backbone.parameters():
        param.requires_grad = False
    for m in model.student_model.backbone.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.eval()
    for layer in cfg.BACKBONE.TRAIN_LAYERS:
        for m in getattr(model.student_model.backbone, layer).modules():
            if isinstance(m, nn.BatchNorm2d):
                m.train()
        for param in getattr(model.student_model.backbone, layer).parameters():
            param.requires_grad = True

    # 学生模型backbone部分参数加入训练
    trainable_params = []
    trainable_params += [{'params': filter(lambda x: x.requires_grad, model.student_model.backbone.parameters()),
                          'lr': cfg.BACKBONE.LAYERS_LR * cfg.TRAIN.BASE_LR}]
    # 学生模型其他部分参数加入训练
    head_params = model.student_model.get_head_parameters()
    for i in range(len(head_params)):
        trainable_params += [{'params': head_params[i], 'lr': cfg.TRAIN.BASE_LR}]
    # adapt层参数加入训练
    trainable_params += [{'params': model.t_adp.parameters(), 'lr': cfg.TRAIN.BASE_LR},
                         {'params': model.s_adp.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
...
def prepare_model(cfg, rep=False):
...
def main():
    ...
    teacher_model = prepare_model(cfg, rep=True)
    teacher_output_channels = cfg.TRAIN.NUM_CHANNELS
    cfg.merge_from_file(dist_settings['student_cfg'])
    student_model = prepare_model(cfg, rep=False)
    student_output_channels = cfg.TRAIN.NUM_CHANNELS
    # dist_kwargs = {'teacher_model': teacher_model, 'student_model': student_model,
    #                'teacher_channel': teacher_output_channels, 'student_channel': student_output_channels}
    # dist_kwargs += dist_settings['distill_kwargs']
    model = FitNetDistiller(teacher_model=teacher_model, student_model=student_model,
                            teacher_channel=teacher_output_channels, student_channel=student_output_channels,
                            **dist_settings['distill_kwargs']).cuda().train()
    ...
    model.student_model.points = torch.from_numpy(points[None, ...]).cuda()
    model.student_model.weights = trial_settings['WEIGHT_SETTINGS']
    model.student_model.ti = trial_settings['UPDATE_SETTINGS']['neg_iou_thresh']
    model.student_model.ts = trial_settings['UPDATE_SETTINGS']['neg_score_thresh']
    model.student_model.logger = logger
    model.teacher_model.points = torch.from_numpy(points[None, ...]).cuda()
    model.teacher_model.weights = trial_settings['WEIGHT_SETTINGS']
    model.teacher_model.ti = trial_settings['UPDATE_SETTINGS']['neg_iou_thresh']
    model.teacher_model.ts = trial_settings['UPDATE_SETTINGS']['neg_score_thresh']
    model.teacher_model.logger = logger
    if cfg.BASE == 'anchor':
        ...
        model.teacher_model.anchors = torch.from_numpy(anchors.all_anchors[1][None, ...]).cuda()
        model.student_model.anchors = torch.from_numpy(anchors.all_anchors[1][None, ...]).cuda()
    elif cfg.BASE == 'point':
...
def train(model, optimizer, lr_scheduler, train_loader, trial_settings,
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCH):
        ...
        if not self_update and (epoch == cfg.SELF_EPOCH or start_epoch >= cfg.SELF_EPOCH):
            ...
            model.student_model.update_settings = trial_settings['UPDATE_SETTINGS']
            model.student_model.weights = trial_settings['WEIGHT_SETTINGS_SELF']
            model.teacher_model.update_settings = trial_settings['UPDATE_SETTINGS']
            model.teacher_model.weights = trial_settings['WEIGHT_SETTINGS_SELF']
        ...
        if get_rank() == 0:
            ...
            torch.save(
                {'state_dict': dist_model.module.student_model.state_dict()},
                cfg.TRAIN.SNAPSHOT_DIR + '/student_checkpoint_e%d.pth' % (epoch + 1))
...
if __name__ == '__main__':
    ...
    logger.info("load distillation setting from {:s}".format(args.config))
    with open(os.path.join(args.config), 'r', encoding='utf-8') as f:
        cond = f.read()
        dist_settings = yaml.load(cond, Loader=yaml.FullLoader)
        f.close()

    # load config
    tracker_name = dist_settings['tracker']
    cfg = get_config(tracker_name)
    cfg.merge_from_file(dist_settings['teacher_cfg'])
