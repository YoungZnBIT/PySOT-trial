...
from utils_script.get_screen_shot import window_capture

torch.set_num_threads(1)

parser = argparse.ArgumentParser(description='tracking demo')
parser.add_argument('--tracker', default='UPDMobileSiam', type=str, help='config file')
parser.add_argument('--config', default='experiments/mobilesiam/trial-test.yaml', type=str, help='config file')
    ...
    if not video_name:
        ...
        while True:
            ...
            if ret:
                ...
                break
    elif video_name == 'screen_shot':
        i = 0
        while True:
            window_capture('z/test/screen_shot_{:d}.jpg'.format(i))
            frame = cv2.imread('z/test/screen_shot_{:d}.jpg'.format(i))
            time.sleep(0.02)
            i += 1
            yield frame
    else:
