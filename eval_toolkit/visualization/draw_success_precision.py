...
def draw(dataset, dataset_name, video_name, eval_trackers=None, width=2, font=1.5,
    ...
    for idx in range(n_frame):
        ...
        # if idx > 0:
        #     idx_ = (idx * 5) + np.random.randint(-4, 4)
        # else:
        #     idx_ = idx
        # cv2.putText(img, str(idx_), (pos, pos), cv2.FONT_HERSHEY_TRIPLEX, font, (0, 255, 255), 2)
        if draw_gt:
            gt_bbox = gt_traj[idx]
...
def draw_success_precision(success_ret, name, videos, attr, precision_ret=None,
    ...
    ax.grid(b=True)
    ax.set_aspect(1)
    plt.xlabel('Overlap threshold', fontsize=x_fontsize, labelpad=1.5)
    plt.ylabel('Success rate', fontsize=y_fontsize, labelpad=1.)
    if attr == 'ALL':
    ...
    if precision_ret:
        ...
        ax.grid(b=True)
        ax.set_aspect(50)
        plt.xlabel('Location error threshold', fontsize=x_fontsize, labelpad=1.5)
        plt.ylabel('Precision', fontsize=y_fontsize, labelpad=1.)
        if attr == 'ALL':
