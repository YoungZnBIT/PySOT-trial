...
parser.add_argument('--config', default='experiments/mobilesiam/trial-conf.yaml', type=str, help='config file')
...
def build_opt_lr(model):
    ...
    for param in model.process_zf.parameters():
        param.requires_grad = False
    for m in model.process_zf.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.eval()

    model.updater = repvgg_model_convert(model.updater)
    for param in model.updater.parameters():
        param.requires_grad = False
    for m in model.updater.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.eval()

    trainable_params = [{'params': model.process_conf.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
