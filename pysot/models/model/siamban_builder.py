...
class SiamBANBuilder(BaseModelBuilder):
    ...
    def forward_param(self, x):
        zf = [torch.randn((1, 256, 7, 7)), torch.randn((1, 256, 7, 7)), torch.randn((1, 256, 7, 7))]
        xf = self.backbone(x)
        if self.cfg.ADJUST.ADJUST:
            xf = self.neck(xf)
    ...
    def forward_trial_old(self, data):
        ...
        with torch.no_grad():
            ...
            if self.train_epoch > 0 and not self.validate:
                iou_weight = sofTmax(-iou * positive[:, None, None], T=0.4, b=-0.25, mask=neg_mask, average='batch')
                if self.train_epoch > 4:
                    neg_weight = sofTmax(score, T=0.4, b=0.20, mask=neg_mask * iou_weight, average='batch')
                else:
                    neg_weight = sofTmax(score, T=0.4, b=0.35, mask=neg_mask * iou_weight, average='batch')
            else:
                neg_weight = label2weight(neg_mask, avg='batch')

            if self.train_epoch > 0 and not self.validate:
                pos_weight_cls = sofTmax(iou, T=0.2, b=0.8, mask=pos_mask, average='batch')
            else:
                pos_weight_cls = label2weight(pos_mask, avg='batch')
    ...
    def forward_trial_abl(self, data):
        template = data['template'].cuda()
        search = data['search'].cuda()
        label_cls = data['label_cls'].cuda()
        label_loc = data['label_loc'].cuda()
        ...
        with torch.no_grad():
            ...
            neg_weight = label2weight(neg_mask, avg='batch')  # neg_weight = label2weight(neg_mask, avg='sample')
            pos_weight_cls = label2weight(pos_mask, avg='batch')
            pos_weight_iou = pos_weight_cls
            pos_weight_l1 = pos_weight_iou
    ...
    def forward_teacher(self, data):
        ...
        outputs = {'teacher_score': score, 'teacher_iou': iou}
        return outputs
    ...
    def forward_student(self, data):
        ...
        with torch.no_grad():
            # 基于self-training方式，根据pred boxes与GT直接的iou以及boxes的score，重新划分正负样本
            if self.update_settings is not None and not self.validate:
                label_cls = label_update_(label_cls.cpu().detach().numpy(), bbox_mask.cpu().detach().numpy(),
                                          score.cpu().detach().numpy(), iou.cpu().detach().numpy(),
                                               positive.cpu().detach().numpy(), **self.update_settings)
            ...
            # pos_weight_l1 = label2weight(pos_mask, avg='batch')
            if self.train_epoch > 0 and not self.validate:
                neg_weight = sofTmax((score - 0.30) * (0.15 - iou * positive[:, None, None]), T=0.25, mask=neg_mask, average='batch')
            else:
                neg_weight = label2weight(neg_mask, avg='batch')

            if self.train_epoch > 0 and not self.validate:
                pos_weight_cls = sofTmax(iou, T=0.2, b=0.7, mask=pos_mask, average='batch')
            else:
                pos_weight_cls = label2weight(pos_mask, avg='batch')

            pos_weight_l1 = label2weight(pos_mask, avg='batch')

            if self.train_epoch > 215 and not self.validate:
                loc_weight = pos_mask + _pos_mask * iou
                pos_weight_iou = sofTmax(loc_weight, T=0.125, b=0.6, mask=bbox_mask, average='batch')
            else:
                pos_weight_iou = pos_weight_l1
        ...
        loc_loss = iou_loss * self.weights['iou_weight']
        if self.train_epoch > 215 and self.weights['l1_weight']:
            loc_loss += l1_loss * self.weights['l1_weight']

        # a0 = data['label_cls'].numpy()
        # a1 = iou.cpu().detach().numpy()
        # a2 = score.cpu().detach().numpy()
        # a4 = bbox_mask.cpu().detach().numpy()
        # a5 = neg_weight.cpu().detach().numpy()
        # import cv2
        # search_img = search.permute((0, 2, 3, 1)).contiguous().type(torch.uint8).cpu().detach().numpy()
        # template_img = template.permute((0, 2, 3, 1)).contiguous().type(torch.uint8).cpu().detach().numpy()
        # j = 2
