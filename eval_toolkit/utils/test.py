...
def test(tracker, name, dataset, test_video='', save_path='results', visual=False, test_name=''):
    ...
    2.LaSOT等速度很慢的超大型数据集，可将其分为若干个子集，分配到各个显卡上并行测试，
      选择base save方式，则会保存在同一文件夹中，最后组成完整的结果
    """
    # 这块的代码忘了啥作用了, 囧...
    # if test_name != '':
    ...
    if 'VOT20' in base_name and 'VOT2018-LT' != base_name and 'VOT2020' != base_name:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            if base_name != 'VOT2018-LT':
                tracker_traj = np.array(pred_bboxes)
                n_frame = len(tracker_traj)
                ...
                cle = np.mean(error_)
                print("CLE:%.2f, precision: %.4f, AUC: %.4f" % (cle, a_p[20], np.mean(a_o)))
    ...
    print("average speed: {:.2f}".format(num_frames / times))
