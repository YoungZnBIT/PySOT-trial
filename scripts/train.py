...
from pysot.datasets.dataset_builder import build_dataset


logger = logging.getLogger('global')
parser = argparse.ArgumentParser(description='siamese tracking')
parser.add_argument('--tracker', default='SiamBAN', type=str, help='config file')
parser.add_argument('--config', default='experiments/siamban/test.yaml', type=str, help='config file')
...
def build_data_loader():
    ...
    train_dataset = build_dataset(tracker_name, cfg)
    ...
    for idx, data in enumerate(train_loader):
        ...
        if idx % num_per_epoch == 0 and idx != 0:
            for index, pg in enumerate(optimizer.param_groups):
                ...
                if rank == 0:
                    tb_writer.add_scalar('lr/group{}'.format(index+1), pg['lr'], tb_idx)
        ...
        if rank == 0:
            ...
            if (idx+1) % cfg.TRAIN.PRINT_FREQ == 0:
                info = "Epoch: [{}][{}/{}] lr: {:.6f}\n".format(epoch+1, (idx+1) % num_per_epoch, num_per_epoch, cur_lr)
...
def main():
    # rank, world_size = dist_init(mode='train')
    rank, world_size = dist_init(mode='debug')
    # rank, world_size = 0, 8
