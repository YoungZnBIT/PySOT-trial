...
class Generator(Dataset):
    def __init__(self,
        ...
        if multi_temp:
            self.mt_settings = aug_settings['multi_temp']
    ...
    def trans_aug(self, trans, video_index, all_boxes, mix_boxes, search_image, search_bbox, search_img, search_box, aug_settings):

        # 将目标区域（带一定大小的背景）从搜索图像中裁剪出来。记录下box的放大倍数，以便移动后精确计算
        if trans < aug_settings['translation_background']['prob']:
            ...
            if trans < self.aug_settings['translation_other']['prob']:
        ...
        elif trans < self.aug_settings['mixup']['prob']:
        ...
        return search_img, search_box, mix_boxes

    def neg_aug(self, neg, video_index, mix_boxes, search_image, search_bbox, search_img, search_box, aug_settings, crop_size, crop_settings):
        pos = 1

        # 随机地在搜索图像中找到一片背景区域作为搜索patch，要求其中不能有目标
        if neg < aug_settings['disappear']['prob']:
            outs = disappear(search_image, search_bbox, size=crop_size, settings=crop_settings)
            ...
            if neg < aug_settings['occ_background']['prob']:
                # 在当前图像中找到一块背景区域作为遮挡块
                occ_image = search_image
                occ_bbox = random_background(occ_image, search_bbox,
                                             min_rate=aug_settings['occ_background']['crop_rate_min'],
                                             max_rate=aug_settings['occ_background']['crop_rate_max'],
                                             protect_settings=rand(
                                                 aug_settings['occ_background']['protect_rate_min'],
                                                 aug_settings['occ_background']['protect_rate_max']))
            # 从其他序列中crop出物体作为遮挡块
            else:
                random_index, random_mixes, random_image, random_bbox = \
                    self.datareader.get_random_data(video_index, read_all_boxes=False, mode=self.mode,
                                                    rate=aug_settings['occ_object']['data_rate'])
                occ_image, occ_bbox = random_image, random_bbox

            # 将遮挡块/物体覆盖在目标上
            if occ_bbox is not None:
                occ_image_, occ_box = random_crop(occ_image, occ_bbox, crop_size, settings=crop_settings)
                # 遮挡块需要带一定背景区域
                occ_box_ = box2roi(occ_box, rate_w=self.roi_rate(), rate_h=self.roi_rate(), boundary=crop_size)
                box = list(map(int, occ_box_))
                # 裁剪出遮挡块
                occ = occ_image_[box[1]: box[3] + 1, box[0]: box[2] + 1]
                # 将遮挡块覆盖到目标框位置上，并计算box被遮挡部分的占比
                image_, occed_box, overlap = random_occ(search_img, search_box, obj=occ,
                                                        center_rate=aug_settings['center_rate'],
                                                        try_num=aug_settings['try_num'],
                                                        overlap_thresh=aug_settings['overlap_thresh'])
                # 当目标大部分被完全覆盖时，才完成有效的遮挡扩增
                if occed_box is not None and overlap > aug_settings['overlap_thresh']:
                    search_img = image_
                    pos = 0
        return search_img, search_box, mix_boxes, pos

    def __getitem__(self, index):
        if self.multi_temp:
            return self.get_multi_temp_data(index)
        else:
            return self.get_pair_data(index)

    def get_pair_data(self, index):
        cv2.setNumThreads(0)
        ...
        if (neg < self.aug_settings['occ_object']['prob']) or (neg < self.aug_settings['neg_pair']['prob']):
            if neg < self.aug_settings['neg_pair']['prob']:
                ...
                search_img, search_box, mix_boxes, pos = self.neg_aug(neg, video_index, mix_boxes,
                                                                      search_image, search_bbox,
                                                                      search_img, search_box,
                                                                      self.aug_settings, self.s_x, self.crop_settings['val'])
            ...
            search_img, search_box, mix_boxes = self.trans_aug(translation, video_index, all_boxes, mix_boxes,
                                                               search_image, search_bbox, search_img, search_box,
                                                               self.aug_settings)
        ...
        if self.aug_settings['gray']['prob'] == 1 or (self.aug_settings['gray']['prob'] and random_sys() < self.aug_settings['gray']['prob']):
    ...
    def get_multi_temp_data(self, index):
        cv2.setNumThreads(0)
        cv2.ocl.setUseOpenCL(False)
        ...
        neg_temp = [random.random() for i in range(self.mt_settings['num_temp'])]
        neg_pair_temp = [x < self.mt_settings['neg_pair']['prob'] for x in neg_temp]
        pos_temp = []
        # 从数据集中读入训练数据：
        # 搜索图像、模板图像、搜索图像上目标的 bbox、模板图像上的 bbox
        while True:
            all_boxes, search_image, search_bbox, template_image, template_bbox, temp_images, temp_bboxes, neg_pair_temp = \
                self.datareader.get_data_mt(video_index, read_all_boxes=self.use_all_boxes, neg_temp=neg_pair_temp,
                                            num_temp=self.mt_settings['num_temp'], sample_range=self.mt_settings['sample_range'])
            if search_image is None or template_image is None:
                video_index = self.datareader.random_choice(mode=self.mode)
            else:
                break
        ...
        temp_boxes = []
        temp_imgs = []
        for i in range(self.mt_settings['num_temp']):
            temp_img, temp_box = random_crop(temp_images[i], temp_bboxes[i],
                                             size=self.s_z, settings=self.crop_settings['multi_temp'])
            ...
            if self.mt_settings['neg_pair']['prob'] < neg_temp[i] < self.mt_settings['occ_object']['prob']:
                temp_imgs[i], temp_boxes[i], _, pos_ = self.neg_aug(neg_temp[i], video_index, None,
                                                                    temp_images[i], temp_bboxes[i],
                                                                    temp_imgs[i], temp_boxes[i],
                                                                    self.mt_settings, self.s_z,
                                                                    self.crop_settings['template'])
                if pos_ == 0:
                    pos_temp[i] = 0.
        ...
        if self.aug_settings['gray']['prob'] == 1 or (self.aug_settings['gray']['prob'] and random_sys() < self.aug_settings['gray']['prob']):
            ...
            search_img = gray_aug(search_img)
            for i in range(self.mt_settings['num_temp']):
                temp_imgs[i] = gray_aug(temp_imgs[i])
                # temp_imgs[i] = temp_imgs[i].transpose((2, 0, 1)).astype(np.float32)

        # for i in range(self.mt_settings['num_temp']):
        #     temp_img_ = cv2.cvtColor(temp_imgs[i].copy(), cv2.COLOR_RGB2BGR)
        #     box = list(map(int, temp_boxes[i]))
        #     cv2.rectangle(temp_img_, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)
        #     cv2.imshow('t' + str(i), temp_img_)
        # temp_img_ = cv2.cvtColor(template_img.copy(), cv2.COLOR_RGB2BGR)
        # box = list(map(int, template_box))
        # cv2.rectangle(temp_img_, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)
        # cv2.imshow('Z', temp_img_)
        # search_img_ = cv2.cvtColor(search_img.copy(), cv2.COLOR_RGB2BGR)
        # box = list(map(int, search_box))
        # cv2.rectangle(search_img_, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)
        # cv2.imshow('XFinal', search_img_)
        # cv2.waitKey()
