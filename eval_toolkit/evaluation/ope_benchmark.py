...
class OPEBenchmark:
    ...
    def eval_cle(self, eval_trackers=None):
        """
        Args:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                gt_center = gt_traj[:, :2] + gt_traj[:, 2:] / 2.
                tracker_center = tracker_traj[:, :2] + tracker_traj[:, 2:] / 2.
                mask = (gt_traj[:, 2] > 0.) * (gt_traj[:, 3] > 0.)
                gt_center = gt_center[mask]
                tracker_center = tracker_center[mask]
                error = gt_center - tracker_center
                error_ = np.sqrt(error[:, 0] ** 2 + error[:, 1] ** 2)
                cle_[video.name] = np.mean(error_)
                cles += np.mean(error_)
            cle[tracker_name] = cle_
            cle[tracker_name]['ALL'] = cles / len(self.dataset)
        return cle
