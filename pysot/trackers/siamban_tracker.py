...
class SiamBANTracker(SiameseTracker):
    def __init__(self, cfg, model):
        super(SiamBANTracker, self).__init__(cfg, model)
    ...
    def init(self, img, bbox, restart=False):
        ...
        # ih, iw = img.shape[:2]
        # if not restart:
        #     self.tiny = (((ih * iw) / (bbox[2] * bbox[3])) > 1700)
    ...
    def track(self, img):
        ...
        # scale penalty
        s_c = change(sz(pred_bbox[2, :], pred_bbox[3, :], context_amount) /
                     (sz(self.size[0] * scale_z, self.size[1] * scale_z, context_amount)))
